...
public class DbOpenHelper {
    ...
    // Insert DB
    public long insertColumn(String potId, boolean wcon, boolean vcon){
        ContentValues values = new ContentValues();
        values.put(Database.CreateDB.POTID, potId);
        values.put(Database.CreateDB.WCON, wcon);
        values.put(Database.CreateDB.VCON, vcon);
        return mDB.insert(Database.CreateDB._TABLENAME0, null, values);
    }

    // Update DB
    public boolean updateColumn(long id, String potId, boolean wcon, boolean vcon){
        ContentValues values = new ContentValues();
        values.put(Database.CreateDB.POTID, potId);
        values.put(Database.CreateDB.WCON, wcon);
        values.put(Database.CreateDB.VCON, vcon);
    ...
    public Cursor sortColumn(String sort){
        Cursor c = mDB.rawQuery( "SELECT * FROM contable ORDER BY " + sort + ";", null);
        return c;
    }
}
